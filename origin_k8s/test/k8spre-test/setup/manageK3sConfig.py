...
def defaultSeedEmulatorDockerDir() -> str:
    ...
    if len(SETUP_DIR.parents) >= 4:
        candidates.append(SETUP_DIR.parents[3] / "seed-emulator/docker_images/multiarch")
...
def fabricNodeRows(data: dict[str, Any], nodes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, node in enumerate(nodes):
        ...
        if not underlay:
            raise SystemExit(
                f"fabric.nodes.{node['name']}.underlayInterface is required for linux-vxlan"
            )
