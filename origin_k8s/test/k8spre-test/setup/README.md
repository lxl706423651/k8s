...
- Kubeconfig: `/home/lxl/k8s/origin_k8s/test/k8spre-test/setup/seedemu-k3s.kubeconfig.yaml`
- 默认实验 namespace: `seedemu-k3s-real-topo`
- K3s 配置: `/home/lxl/k8s/origin_k8s/test/k8spre-test/setup/configK3s.yaml`

`buildK3sCluster.sh` 成功后，可以直接使用：

```bash
export KUBECONFIG="/home/lxl/k8s/origin_k8s/test/k8spre-test/setup/seedemu-k3s.kubeconfig.yaml"
...
两台真实物理机可以使用内置 linux-vxlan fabric 脚本准备统一父接口。例如：
...
如果 `fabric.type=ovn`，不需要手动配置 `br-seedemu/vxseed0`。`buildK3sCluster.sh` 会在 K3s 和 Multus 安装后调用：
