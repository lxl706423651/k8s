...
RUNNER="${SCRIPT_DIR}/11_run_full_flow_12node.sh"

if [ -z "${EXPERIMENT_DIR:-}" ]; then
  echo "Error: EXPERIMENT_DIR is not set."
  echo "Example:"
  echo "  export EXPERIMENT_DIR=/home/lxl/k8s/lxl/logs/20260417_xxxxxx_9955"
  exit 1
fi
...
nohup bash -lc "
  set -euo pipefail
  cd '${SCRIPT_DIR}'
  source '${SCRIPT_DIR}/env_12node.sh' >/dev/null 2>&1
