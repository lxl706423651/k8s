...
#
# Inputs: an experiment directory containing output/k8s.yaml and
# output/build_images.sh. Build/preload tuning constants are defined below.
...
# Context: run from the controller host after compile.sh has produced hard
...
SEED_BUILD_PARALLELISM=12
SEED_DOCKER_BUILDKIT=1
SEED_DOCKER_BUILDX=1
SEED_BUILD_SKIP_EXISTING=0
SEED_FAST_IMAGE_ARCHIVE=1
SEED_FAST_ARCHIVE_BASE_IMAGE=""
SEED_COMPOSE_DOCKER_CLI_BUILD=1
SEED_PRELOAD_NODE_CONCURRENCY=4
SEED_PRELOAD_IMAGE_CONCURRENCY=2
SEED_PRELOAD_RETRIES=3
SEED_PRELOAD_BACKOFF_SECONDS=5
SEED_REGISTRY_PUSH_RETRIES=5
SEED_REGISTRY_PUSH_BACKOFF_SECONDS=5
SEED_REGISTRY_PUSH_TIMEOUT_SECONDS=180
export SEED_BUILD_PARALLELISM SEED_DOCKER_BUILDKIT SEED_DOCKER_BUILDX
export SEED_BUILD_SKIP_EXISTING SEED_FAST_IMAGE_ARCHIVE SEED_FAST_ARCHIVE_BASE_IMAGE
export SEED_COMPOSE_DOCKER_CLI_BUILD SEED_PRELOAD_NODE_CONCURRENCY
export SEED_PRELOAD_IMAGE_CONCURRENCY SEED_PRELOAD_RETRIES SEED_PRELOAD_BACKOFF_SECONDS
...
FAST_ARCHIVE_SCRIPT="${TEST_DIR}/fastSeedemuImageArchive.py"
...
echo "SEED_FAST_IMAGE_ARCHIVE=${SEED_FAST_IMAGE_ARCHIVE:-0}"
...
echo "SEED_REGISTRY_PUSH_RETRIES=${SEED_REGISTRY_PUSH_RETRIES}"
echo "SEED_REGISTRY_PUSH_TIMEOUT_SECONDS=${SEED_REGISTRY_PUSH_TIMEOUT_SECONDS}"
...
tar -C "${NODE_IMAGE_VALIDATE_DIR}" -czf /tmp/node_image_refs.tar.gz .
...
scp -q "${SSH_OPTS[@]}" /tmp/node_image_refs.tar.gz "${SEED_K3S_USER}@${SEED_MASTER_NODE_IP}:${REMOTE_WORK_DIR}/"
if [ "${SEED_FAST_IMAGE_ARCHIVE:-0}" = "1" ]; then
    require_file "${FAST_ARCHIVE_SCRIPT}"
    scp -q "${SSH_OPTS[@]}" "${FAST_ARCHIVE_SCRIPT}" "${SEED_K3S_USER}@${SEED_MASTER_NODE_IP}:${REMOTE_WORK_DIR}/"
fi
...
DOCKER_BUILDKIT="$3"
COMPOSE_DOCKER_CLI_BUILD="$4"
SEED_IMAGE_DISTRIBUTION_MODE="$5"
SEED_REGISTRY="$6"
SEED_REGISTRY_LOCAL_ENDPOINT="$7"
SEED_REGISTRY_PUSH_RETRIES="$8"
SEED_REGISTRY_PUSH_BACKOFF_SECONDS="$9"
SEED_REGISTRY_PUSH_TIMEOUT_SECONDS="${10}"
SEED_DOCKER_BUILDX="${11}"
SEED_BUILD_SKIP_EXISTING="${12}"
SEED_FAST_IMAGE_ARCHIVE="${13:-0}"
SEED_FAST_ARCHIVE_BASE_IMAGE="${14:-}"
cd "${REMOTE_WORK_DIR}"
...
export SEED_FAST_IMAGE_ARCHIVE="${SEED_FAST_IMAGE_ARCHIVE}"
...
install_buildx_wrapper
if [[ "${SEED_FAST_IMAGE_ARCHIVE}" == "1" ]]; then
  if [ ! -f "${REMOTE_WORK_DIR}/fastSeedemuImageArchive.py" ]; then
    echo "Missing fastSeedemuImageArchive.py" >&2
    exit 1
  fi
  rm -rf "${REMOTE_WORK_DIR}/node_image_refs" "${REMOTE_WORK_DIR}/node_archives"
  mkdir -p "${REMOTE_WORK_DIR}/node_image_refs" "${REMOTE_WORK_DIR}/node_archives"
  tar -xzf "${REMOTE_WORK_DIR}/node_image_refs.tar.gz" -C "${REMOTE_WORK_DIR}/node_image_refs"
  echo "[build] fast archive mode enabled"
  fast_args=(
    python3 "${REMOTE_WORK_DIR}/fastSeedemuImageArchive.py"
    --work-dir "${REMOTE_WORK_DIR}"
    --node-image-dir "${REMOTE_WORK_DIR}/node_image_refs"
    --archive-dir "${REMOTE_WORK_DIR}/node_archives"
    --summary "${REMOTE_WORK_DIR}/fast_archive_summary.json"
  )
  if [ -n "${SEED_FAST_ARCHIVE_BASE_IMAGE}" ]; then
    fast_args+=(--base-image "${SEED_FAST_ARCHIVE_BASE_IMAGE}")
  fi
  "${fast_args[@]}" 2>&1 | tee "${REMOTE_WORK_DIR}/build.log"
elif [ -x "./build_images.sh" ]; then
...
printf -v REMOTE_CMD 'sudo -n bash %q %q %q %q %q %q %q %q %q %q %q %q %q %q %q' \
    "${REMOTE_SCRIPT}" \
    "${REMOTE_WORK_DIR}" \
...
PRELOAD_RETRIES="${SEED_PRELOAD_RETRIES}"
PRELOAD_BACKOFF_SECONDS="${SEED_PRELOAD_BACKOFF_SECONDS}"
PRELOAD_NODE_CONCURRENCY="${SEED_PRELOAD_NODE_CONCURRENCY}"
PRELOAD_IMAGE_CONCURRENCY="${SEED_PRELOAD_IMAGE_CONCURRENCY}"
...
if [ "${SEED_FAST_IMAGE_ARCHIVE:-0}" = "1" ]; then
    echo "Importing per-node image archives into K3s containerd..."
    import_archive_on_node() {
        local node_ip="$1"
        local node_label="$2"
        local log_file="${EXPERIMENT_DIR}/preload_${node_label}.log"
        local remote_archive="${REMOTE_WORK_DIR}/node_archives/images_${node_label}.tar"

        : > "${log_file}"
        if ! ssh "${SSH_OPTS[@]}" "${SEED_K3S_USER}@${SEED_MASTER_NODE_IP}" \
            "sudo -n test -s '${remote_archive}'" >> "${log_file}" 2>&1; then
            echo "[preload] skip: no archive assigned to ${node_label}" | tee -a "${log_file}"
            return 0
        fi
        if [ "${node_ip}" = "${SEED_MASTER_NODE_IP}" ]; then
            ssh "${SSH_OPTS[@]}" "${SEED_K3S_USER}@${node_ip}" \
                "sudo -n k3s ctr images import '${remote_archive}'" >> "${log_file}" 2>&1
        else
            ssh "${SSH_OPTS[@]}" "${SEED_K3S_USER}@${SEED_MASTER_NODE_IP}" \
                "sudo -n cat '${remote_archive}'" \
                | ssh "${SSH_OPTS[@]}" "${SEED_K3S_USER}@${node_ip}" \
                    "sudo -n k3s ctr images import -" >> "${log_file}" 2>&1
        fi
        echo "✅ ${node_label} image archive imported" | tee -a "${log_file}"
    }
    for i in "${!SEED_NODE_NAMES[@]}"; do
        import_archive_on_node "${SEED_NODE_IPS[$i]}" "${SEED_NODE_NAMES[$i]}" &
        pending_pids+=("$!")
