...
#
# Side effects:
#   - Runs each host-local kvm/destroyKvmVms.sh on its owning hypervisor.
...
destroyHostVms() {
    ...
    runHostCommand "${name}" "${ip}" "${connection}" "${ssh_user}" "${ssh_key}" "
        set -euo pipefail
        if [ -x $(printf '%q' "${remote_work_dir}/kvm/destroyKvmVms.sh") ] && [ -s $(printf '%q' "${remote_work_dir}/kvmState.yaml") ]; then
            cd $(printf '%q' "${remote_work_dir}")
            bash ./kvm/destroyKvmVms.sh ./kvmState.yaml
        fi
